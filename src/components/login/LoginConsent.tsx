 ...
 *
 * สำคัญ: ระบบบัญชีจริง (Supabase Auth + ฐานข้อมูลความก้าวหน้า) ยังไม่ถูกสร้าง
 * ปุ่มนี้จึงบอกตรง ๆ ว่ายังใช้งานจริงไม่ได้แทนที่จะแกล้งพาไปหน้าที่ไม่มีอะไรจริงอยู่ข้างหลัง
 * (ตรงกับกติกาโครงการ: ห้ามทำ Fake Function / Demo แล้วอ้างว่าใช้งานได้จริง)
 *
 * กล่องกาถูกไม่ผูกกับอะไรที่ทำงานจริงตอนนี้ — มีไว้ให้เห็นหน้าตาของขั้นตอนที่จะเกิดขึ้นจริง
 * เมื่อต่อ Auth เสร็จ: ต้องกาก่อนปุ่ม Google ถึงจะกดได้
 */
    ...
    <div className="w-full max-w-[400px] rounded-2xl border border-hero-line bg-hero-bg/70 p-7 backdrop-blur-xl sm:p-8">
      ...
      {tried ? (
        <p className="m-0 mt-3 rounded-lg border border-warn/40 bg-hero-bg/60 px-3 py-2.5 text-[12px] leading-relaxed text-hero-ink-2">
          ระบบเข้าสู่ระบบด้วย Google กำลังอยู่ระหว่างการพัฒนา ยังใช้งานจริงไม่ได้ในตอนนี้
        </p>
      ) : null}
